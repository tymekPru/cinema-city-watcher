...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.film_id = env(
            "FILM_ID", ""
        )  # np. 7268s2r; ma pierwszeństwo nad FILM_MATCH
        self.film_match = env(
            "FILM_MATCH", "odys"
        ).lower()  # fragment nazwy filmu (case-insensitive)
        self.required_attrs = [
            a.strip() for a in env("REQUIRED_ATTRS", "70-mm").split(",") if a.strip()
        ]
        self.horizon_days = int(env("HORIZON_DAYS", "45"))
        self.request_gap_s = float(env("REQUEST_GAP_S", "0.15"))
        self.min_ratio_delta = float(
            env("MIN_RATIO_DELTA", "0.002")
        )  # 1 miejsce = 1/385 ≈ 0.0026
